# Runbook — sdkwork-notes deploy / upgrade / rollback (EN)
...
targets. Image reference: `registry.sdkwork.com/apps/sdkwork-notes-standalone:0.1.0`
(tag from `sdkwork.app.config.json` → `release.currentVersion`).

> ℹ️ **Delivery posture**: standalone delivery is a host package (`host-package/nginx`); cloud delivery is a container image (`container-image/kubernetes`).
...
install syncs the bundle to `/opt/deploy/sdkwork-notes/bundle`, loads the image, starts
